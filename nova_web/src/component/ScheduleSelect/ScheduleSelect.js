  ...
  }) 
{
  ...
  const handleScheduleDateTimeChange = (startDate, endDate, startTime, endTime) => {
    const updatedSchedules = [...sendScheduleData.schedules];
  ...
  const daysInMonth = startPickerValue.year && startPickerValue.month
  ...
  const validateAndSetPickerValue = (
    year,
    month,
    day,
    setPickerValue
  ...
  }, [startPickerValue, endPickerValue, startTimePickerValue, endTimePickerValue]);
  ...
  useEffect(() => {
    ...
    handleScheduleDateTimeChange(targetStartDate, targetEndDate, targetStartTime, targetEndTime)
  }, [startPickerValue, endPickerValue, startTimePickerValue, endTimePickerValue]);
        ...
        { index !== 0 ? (
          <div className={style["top-delete-box"]}>
            <div className={style["remove-schedule-button"]}>
              <p className={style["remove-schedule-text"]} onClick={()=>{
...
function DatePicker({ year, month, day,
   ...
   isYearPickerOpen, isMonthPickerOpen, isDayPickerOpen, type,
    weekDay
  }) {
  const selectDate = new Date(year, month - 1, day);
...
function TimePicker({ pickerValue, setIsOpen, id, isOpen}) {
  return (
    <div className="DatePicker__container">
      <TimeInfo {...pickerValue} id={id} onOpen={() => setIsOpen(true)} isOpen={isOpen}/>
    </div>
  );
}
  ...
  return (
    <div
      ...
      <div className="DatePicker" >
        ...
        <div className="ok-button" onClick={onClose}>확인</div>
      </div>
    </div>
  );
}
